...
- **跨会话持续开发**：新对话只自动恢复有界的项目滚动状态和最近增量；更早 handoff 仍保存在项目里，需要时再按路径读取，避免历史越长启动上下文越大。
...
| 工具 | 作用 |
| --- | --- |
| `history_session_bootstrap` | 新对话开始时初始化或恢复项目会话；返回有界 `project_state`、`latest_delta` 以及稳定的 `session_key` / `current_path`，不会把全部旧历史复制进新 handoff |
...
对话 1：分析项目 → 修改代码 → 运行测试 → 保存检查点
                                      ↓
对话 2：读取 project_state + latest_delta → 从上次进度继续 → 保存新检查点
```

历史文件使用可读的 Markdown 格式，可以随项目备份或纳入 Git，也方便开发者直接审阅和修订。新 handoff 只记录当前会话本身，不再嵌入“继承的历史摘要”；bootstrap 优先读取 `docs/history-state/PROJECT_STATE.md` 作为滚动状态，并只携带有界的最近增量。旧 handoff 始终保留在 `docs/history-session/`，需要历史证据时由 Agent 按具体路径读取。检查点采用幂等写入，并要求返回 `ok=true` 且会话目标一致后才确认保存成功。
